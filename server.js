...
function handleNewUser(socket) {
  ...
  // If 4 players are already in the game...
    ...
    const message = { type: SERVER.MESSAGE.GAME_FULL };
    socket.send(JSON.stringify(message));
  }
}
...
function passThePotatoTo(newPotatoHolderIndex) {
  ...
  const message = { type: SERVER.BROADCAST.NEW_POTATO_HOLDER, payload: { newPotatoHolderIndex } };
  broadcast(message);
}
...
function startTimer() {
  ...
  const interval = setInterval(() => {
    if (clockValue > 0) {
      ...
      const message = { type: SERVER.BROADCAST.COUNTDOWN, payload: { clockValue } };
      broadcast(message);
    ...
    // At 0...
      ...
      const message = { type: SERVER.BROADCAST.GAME_OVER };
      broadcast(message);
    }
  }, 1000);
}
